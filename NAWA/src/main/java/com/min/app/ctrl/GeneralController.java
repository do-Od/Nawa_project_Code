		...
		if (session.getAttribute("search") == null) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if(ef_title.contains("\n" + 
						"		")) {
					map.add("{ "+"content : '<div> <a href=\"../event/detail?ef_id="+ef_id+"\">"+ef_title.replaceAll("\n" + 
							"		", " ")+"</a></div>', latlng : new kakao.maps.LatLng("+list.get(i).getEf_lc_detail()+") }");
				}else {
					map.add("{ "+"content : '<div> <a href=\"../event/detail?ef_id="+ef_id+"\">"+list.get(i).getEf_title()+"</a> </div>', latlng : new kakao.maps.LatLng("+list.get(i).getEf_lc_detail()+") }");
				}
			}
			pdto.setTotal(e_service.CountList());
			...
			for (int i = 0; i < lists.size(); i++) {
				...
				if(ef_title.contains("\n" + 
					...
					map.add("{ "+"content : '<div> <a href=\"../event/detail?ef_id="+ef_id+"\">"+lists.get(i).getEf_title()+"</a> </div>', latlng : new kakao.maps.LatLng("+lists.get(i).getEf_lc_detail()+") }");
				}
			}	
			model.addAttribute("flist",flist);
